...
#ifdef C_FUNCS
bool C_compress_file(FILE *src, FILE *dst)
{
...
bool C_decompress_file(FILE *src, FILE *dst)
{
    uint8_t inbuff[CHUNK_SIZE];
    uint8_t outbuff[CHUNK_SIZE];
    z_stream stream = {0};
...
bool C_compress_file(const char *src, const char *dst)
{
    FILE *src_file = fopen(src, "r");
    FILE *dst_file = fopen(dst, "w+");
    bool result = C_compress_file(src_file, dst_file);
    delete src_file;
    delete dst_file;
    return result;
}

bool C_decompress_file(const char *src, const char *dst)
{
    FILE *src_file = fopen(src, "r");
    FILE *dst_file = fopen(dst, "w+");
    bool result = C_decompress_file(src_file, dst_file);
    delete src_file;
    delete dst_file;
    return result;
}
// TODO
const char *C_sha256(const uint8_t *data, size_t size)
{
    int h0 = 0x6a09e667;
    int h1 = 0xbb67ae85;
    int h2 = 0x3c6ef372;
...
XXH128_hash_t C_xxhash_file(FILE *f)
{
    XXH3_state_t *state = XXH3_createState();
    if (state == NULL)
    {
        fprintf(stderr, "Hash out of memory!\n");
        return XXH128_hash_t{XXH64_hash_t{0}, XXH64_hash_t{0}};
    }
    XXH3_128bits_reset(state);
    char buff[4096];
    size_t count;
    while ((count = fread(buff, 1, sizeof(buff), f)) != 0)
    {
        XXH3_128bits_update(state, buff, count);
    }
