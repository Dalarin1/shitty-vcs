...
enum class EditType : uint8_t { Match = 0, Insert = 1, Delete = 2 };
...
struct DiffOp {
    ...
    static bool _elem_writer(const DiffOp &diffop, std::ofstream &file) {
        return !file.write(reinterpret_cast<const char *>(&diffop.type), sizeof(EditType))
                    .fail() &&
               !file.write(reinterpret_cast<const char *>(&diffop.pos), sizeof(uint32_t))
                    .fail() &&
               !file.write(reinterpret_cast<const char *>(&diffop.byte), sizeof(uint8_t))
                    .fail();
    }
    static bool _elem_reader(DiffOp &diffop, std::ifstream &file) {
        return !file.read(reinterpret_cast<char *>(&diffop.type), sizeof(EditType))
                    .fail() &&
               !file.read(reinterpret_cast<char *>(&diffop.pos), sizeof(uint32_t))
                    .fail() &&
               !file.read(reinterpret_cast<char *>(&diffop.byte), sizeof(uint8_t)).fail();
    }
};
...
std::vector<DiffOp> myers_diff(const std::vector<char> &a, const std::vector<char> &b) {
    ...
    for (int d = (int)trace.size() - 1; d >= 0; --d) {
        ...
        while (x > prev_x && y > prev_y) {
            ...
            --y;
            ops.push_back(
                {EditType::Match, static_cast<uint32_t>(x), static_cast<uint8_t>(a[x])});
        }

        if (down) {
            --y;
            ops.push_back(
                {EditType::Insert, static_cast<uint32_t>(x), static_cast<uint8_t>(b[y])});
        } else {
            --x;
            ops.push_back(
                {EditType::Delete, static_cast<uint32_t>(x), static_cast<uint8_t>(a[x])});
        }
    }
...
void apply_diff(const std::vector<char> &base, const std::vector<DiffOp> &diff,
                std::vector<char> &out) {
    out.clear();
    size_t base_pos = 0;
...
void print_diff_colored(const std::vector<DiffOp> &diff) {
    ...
    for (const auto &op : diff) {
        switch (op.type) {
        case EditType::Insert:
            std::cout << ANSI_GREEN << "+ " << (isprint(op.byte) ? char(op.byte) : '.')
                      << " (pos " << op.pos << ")" << ANSI_RESET << "\n";
            break;
        case EditType::Delete:
            std::cout << ANSI_RED << "- " << (isprint(op.byte) ? char(op.byte) : '.')
                      << " (pos " << op.pos << ")" << ANSI_RESET << "\n";
            break;
        case EditType::Match:
            std::cout << ANSI_GRAY << "  " << (isprint(op.byte) ? char(op.byte) : '.')
                      << " (pos " << op.pos << ")" << ANSI_RESET << "\n";
            break;
        }
    }
